...
export class WebService {
    ...
    private commentsUrl = 'http://localhost:9000/file-explorer-service';
    ...
    // Add a new comment
    getFiles(filePath: string): Observable<Comment[]> {
        let temp = {"path":filePath};
        let bodyString = JSON.stringify(temp); // Stringify payload
        let headers = new Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
        let options = new RequestOptions({ headers: headers }); // Create a request option
